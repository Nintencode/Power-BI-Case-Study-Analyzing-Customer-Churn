...
All files including the Power BI workbook, data excel sheet, and dashboard graphics are all uploaded in the repository available for review. You can also click on the images in this report to open the image link for a closer/clearer view. 
...
Churn rises steadily with age. Senior customers churn at about 38%, compared to roughly 23% to 25% for younger groups. Breaking age into 10 year bins made the trend even clearer. Churn rate climbs fairly steadily from the 20s through 50s, with a noticeable jump in the 60s and up, even though the customer base itself is concentrated in the 20 to 50 range.
...
Contract length turned out to be one of the strongest factors in the whole dataset. Month to month customers churn at around 46%, compared to under 7% for customers on yearly contracts. Gender did not show a meaningful difference within either group.
...
Somewhat counterintuitively, customers on an unlimited data plan churn more than those without one, 32% versus 16%. That pattern holds even after breaking usage into consumption tiers, so the unlimited plan itself seems tied to churn risk rather than how much data someone actually uses. However, those that use over 10 GB of data on average are slightly more likely to be retained.
...
Customers who have an international plan but are not actively using it churn at a very high rate, over 71%, while customers who are actively using their international plan churn far less. Geographically, churn is fairly consistent across most states, with one state, California, standing out with a noticeably higher rate of 63.24%.
